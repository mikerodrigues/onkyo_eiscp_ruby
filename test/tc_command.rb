...
class TestCommand < Test::Unit::TestCase
  ...
  def test_command_value_to_value_name
    assert_equal(EISCP::Command.command_value_to_value_name("PWR", "01"), "on")
  end

  def test_command_value_name_to_value
    assert_equal(EISCP::Command.command_value_name_to_value("PWR", "on"), "01")
  end
  ...
  def test_parse_system_power
    assert_equal(EISCP::Command.parse('system-power on'), EISCP::Message.parse('PWR01'))
  end

  def test_parse_zone2_system_power
    assert_equal(EISCP::Command.parse('zone2 power on'), EISCP::Message.parse('ZPW01'))
  end
